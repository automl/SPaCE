...
class SACInterface(AgentInterface):
    ...
    def estimate_value_internal(self, inputs):
        return np.squeeze(
            self.learner.sess.run(
                [self.learner.step_ops[6]], {self.learner.observations_ph: inputs}
            )
        )
...
class TRPOInterface(AgentInterface):
    ...
    def mean_policy_std(self, cb_args, cb_kwargs):
        log_std = np.squeeze(
            self.learner.sess.run(
                [self.learner.policy_pi.proba_distribution.logstd],
                {self.learner.policy_pi.obs_ph: np.zeros((1, self.obs_dim))},
            )[0]
        )
...
class PPOInterface(AgentInterface):
    ...
    def mean_policy_std(self, cb_args, cb_kwargs):
        log_std = np.squeeze(
            self.learner.sess.run(
                [self.learner.train_model.proba_distribution.logstd],
                {self.learner.train_model.obs_ph: np.zeros((1, self.obs_dim))},
            )[0]
        )
        return np.mean(np.exp(log_std))
...
class Learner(Enum):
    ...
    def create_learner(self, env, parameters):
        ...
        if self.trpo():
            model = TRPO(
                MlpPolicy, env, **parameters["common"], **parameters[str(self)]
            )
            ...
            model = SAC(
                SACMlpPolicy, env, **parameters["common"], **parameters[str(self)]
            )
...
class ExperimentCallback:
    ...
    def __call__(self, *args, **kwargs):
        if self.algorithm_iteration % self.step_divider == 0:
            ...
            if self.sp_teacher is not None:
                if (
                    self.iteration >= self.n_offset
                    and self.iteration % self.n_inner_steps == 0
                ):
                    vf_inputs, contexts = self.env_wrapper.get_context_buffer()
                    self.sp_teacher.update_distribution(
                        mean_disc_rew, contexts, self.learner.estimate_value(vf_inputs)
                    )
                context_mean = self.sp_teacher.context_dist.mean()
                context_std = np.sqrt(
                    np.diag(self.sp_teacher.context_dist.covariance_matrix())
                )
            ...
            if self.iteration % self.save_interval == 0:
                iter_log_dir = os.path.join(
                    self.log_dir, "iteration-" + str(self.iteration)
                )
...
class AbstractExperiment(ABC):
    ...
    def __init__(
        self, base_log_dir, curriculum_name, learner_name, parameters, seed, view=False
    ):
    ...
    def process_parameters(self):
        allowed_overrides = {
            ...
            "AG_P_RAND": float,
            "AG_FIT_RATE": int,
            "AG_MAX_SIZE": self.parse_max_size,
            "GG_NOISE_LEVEL": float,
            "GG_FIT_RATE": int,
            "GG_P_OLD": float,
        }
    ...
    def get_log_dir(self):
        override_appendix = create_override_appendix(
            self.APPENDIX_KEYS["default"], self.parameters
        )
        ...
        return os.path.join(
            ...
            self.get_env_name(),
            str(self.curriculum),
            leaner_string + override_appendix + self.get_other_appendix(),
            "seed-" + str(self.seed),
        )
    ...
    def train(self):
        ...
        if os.path.exists(log_directory):
            ...
            callback = ExperimentCallback(
                log_directory=log_directory, **callback_params
            )
            model.learn(
                total_timesteps=timesteps, reset_num_timesteps=False, callback=callback
            )
    ...
    def evaluate(self):
        ...
        unsorted_iterations = np.array(
            [int(d[len("iteration-") :]) for d in iteration_dirs]
        )
        ...
        if self.curriculum.self_paced() and not os.path.exists(
            os.path.join(log_dir, "kl_divergences.pkl")
        ):
            kl_divergences = []
